...
public class LinearDecisionRule {
    ...
    //objects with values1 cumulatively dominates object with values2
    //weights don't need to be ordered
    public static boolean exhibitsApproximateCumulativeDominance(double[] weights, double[] values1, double[] values2,
                                                                 int betterIn, int worseIn){
        double[] weightedDelta = new double[weights.length];
        double[] unsignedWeights = new double[weights.length];
        double[] weightSigns = new double[weights.length];
        ...
        int[] orderedIndicesWeightedDelta = Compute.orderedIndices(weightedDelta);
        for (int i = 0; i < betterIn; i++) {
            if(weightedDelta[orderedIndicesWeightedDelta[i]] <= 0){
                return false;
            }
        }
        if(weightedDelta[orderedIndicesWeightedDelta[weightedDelta.length -1 - worseIn]] < 0){
            return false;
        }
        return true;
    }
    ...
    public static boolean[] paretoDominanceApprSet(double[] weights, double[][] objects, int betterIn, int worseIn){
        boolean[] pareto = new boolean[objects.length];
        for (int i = 0; i < pareto.length; i++) {
            pareto[i] = true;
        }
        for (int i = 0; i < objects.length; i++) {
            for (int j = 0; j < objects.length; j++) {
                if(pareto[i] && pareto[j]){
                    double[] featuresValuesI = objects[i];
                    double[] featuresValuesJ = objects[j];
                    if(exhibitsApproximateDominance(weights, featuresValuesI, featuresValuesJ, betterIn, worseIn)){
                        pareto[j] = false;
                    }else if(exhibitsApproximateDominance(weights, featuresValuesJ, featuresValuesI, betterIn, worseIn)){
                        pareto[i] = false;
                    }
                }
            }
